---
name: "translate"
description: "Multilingual app architecture with URL locales, translation files, and SEO rules. Invoke when adding i18n, localization, or translated UI."
---
...
- every project must ship with these locales: `uk`, `en`, `es`, `pt`, `ru`.
...
- required baseline locales in every project: `uk`, `en`, `es`, `pt`, `ru`.
...
- Store messages in per-locale files (`uk.json`, `en.json`, etc.).
...
3. Keep locale-dependent logic explicit (for example currency, support links, legal texts, and date formatting).
...
1. Define locales and fallback rules.
   Minimum required locales: `uk`, `en`, `es`, `pt`, `ru`.
