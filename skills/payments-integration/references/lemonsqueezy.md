...
Universal guide for integrating payments via LemonSqueezy. In this setup it can be used for both `UAH` and `USD`, depending on how the store, product, and variant currency are configured.
...
**Key points:**
- `custom_price` — price in the minor units of the configured checkout currency. For USD, `1500` means `$15.00`
...
- make sure the store/variant configuration matches the currency you expose to the user, for example `UAH` or `USD`
